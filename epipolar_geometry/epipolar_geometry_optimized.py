...
def arguments_parser():
    ...
    parser.add_argument("--max_depth", default=20, type=int, help = "Maximum depth of the image")
    parser.add_argument("--min_depth", default=4, type=int, help = "Minimum depth of the image")
...
def epipolar_line_cal(R, t, x, y):
    t_normalized = t / np.sqrt(np.dot(t, t))  #Normalizing the t vector
    ...
    x_prime_vector = np.matmul(R, x_vector) + t
    x_prime_vector /= np.sqrt(np.dot(x_prime_vector, x_prime_vector))
    x_prime_vector_scaled = np.matmul(R, x_vector_scaled) + t
    x_prime_vector_scaled /= np.sqrt(np.dot(x_prime_vector_scaled, x_prime_vector_scaled))

    #Calculating the normal vector to define the plane
    normal_vector = np.cross(t_normalized, x_prime_vector)
    ...
    coplanar_points_idx = np.where(abs(np.dot(unit_cartesian_vector1.T, normal_vector))<0.01)
...
def click(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        epipolar_line, coplanar_points, x_prime_vector, x_prime_vector_scaled = epipolar_line_cal(R, t, x, y)
...
def main():
    global img1, img2, imgWidth, imgHeight,\
         coord_convert, unit_cartesian_vector1, theta, max_depth, min_depth, R1, t1, R2, t2
